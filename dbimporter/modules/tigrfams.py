...
def import_results(data: RecordData) -> None:
    ...
    if not results:
        return
    for domain in data.record.get_antismash_domains_by_tool(
        tigrfam.__name__.split(".")[-1]
    ):
...
def get_tigrfam_id(cur, identifier: str) -> id:
    ...
    assert identifier
    cur.execute(
        "SELECT tigrfam_id FROM antismash.tigrfams WHERE tigrfam_id = ?", (identifier,)
    )
    ret = cur.fetchone()
    if ret is None:
