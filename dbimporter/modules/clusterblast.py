...
def _get_reference_region_id(cursor, accession: str, cluster_label: str) -> int:
    ...
    cursor.execute(
        "SELECT region_id FROM antismash.regions WHERE accession = ? AND start_pos = ? AND end_pos = ?",
        (accession, start, end),
    )
    ret = cursor.fetchone()
    if ret is None:
...
def store_clusterblast(data: RecordData, results, algorithm):
    ...
    data.cursor.execute(
        "SELECT algorithm_id FROM antismash.clusterblast_algorithms WHERE name = ?",
        (algorithm,),
    )
